...
(function() {
  var IGNORED_ELEMENTS, ZOOM_LEVEL_KEY, changeFont, getKeyFromBackground, pixelValue, totalRatio, zoomLevel;

  totalRatio = 1;

  ZOOM_LEVEL_KEY = 'zoomLevel';

  IGNORED_ELEMENTS = ['script', 'noscript', 'link', 'br', 'embed', 'iframe', 'img', 'video', 'canvas', 'style', '#gritter-notice-wrapper'];
  ...
  changeFont = function(ratioDiff, notification) {
    var call, changeFontSizeCalls, relevantElements, start, _i, _len;
    ...
    _.each(relevantElements, function(element) {
      var fontSize, jqElement, lineHeight;
      if (Util.isBlank(element.innerText) && !(element.tagName === 'INPUT' || element.tagName === 'TEXTAREA')) {
        return;
      }
      jqElement = $(element);
      ...
      return changeFontSizeCalls.push(function() {
        ...
        if (lineHeight != null) {
          return jqElement.css('line-height', pixelValue(lineHeight));
        }
      });
    });
