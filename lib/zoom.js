...
(function() {
  var ZOOM_LEVEL_KEY, changeFont, getKeyFromBackground, ignore, multiplyByRatio, totalRatio, zoomLevel;
  ...
  multiplyByRatio = function(value) {
    return (parseInt(value) * totalRatio).toFixed(2) + 'px';
  };
    ...
    _.each(relevantElements, function(element) {
      ...
      if (!Util.isBlank(element.innerText) || (tagName === 'TEXTAREA')) {
        ...
        if (currentLh.indexOf('px') !== -1) {
          lineHeight = multiplyByRatio(currentLh);
        }
      }
